...
parser.add_argument("--results", "-r", dest="results", required=True, help="path to the file with all the smORFs of a Bacteria", nargs=1)
...
smORFS_x_genome = (len(df) / args.number_of_genomes[0])
...
smorfs50_x_genome = (len(genes50) / args.number_of_genomes[0])
smorfs50_x_genome = round(smorfs50_x_genome)

# Calculate the average of conserved, core, and soft core genes in a single genome
conserved_x_genome = (len(conserved) / args.number_of_genomes[0])
conserved_x_genome = round(conserved_x_genome)

core_x_genome = (len(core) / args.number_of_genomes[0])
core_x_genome = round(core_x_genome)

soft_core_x_genome = (len(soft_core) / args.number_of_genomes[0])
